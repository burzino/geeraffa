...
lst_team = soup.findAll('span', {'class', 'team-name js-team-name'})
...
for i in range (0,len(lst_team)):
    external_span = lst_team[i]
    unwanted = external_span.find('span')
    unwanted.extract()
    lst_teamTag.append(external_span.text.strip().upper())


def rimuoviTagGironi(str):
    external_span = str
    unwanted = external_span.find('span')
    unwanted.extract()
    return external_span.text.strip()

#stampo la classifica dettagliata utiizzando gli array caricati in precedenza
def stampaClassifica():
    str_fileOut = ''
    i = 0
    for titoli in soup.findAll('span', {'class','group-name'}):
        #str_fileOut = str_fileOut + '\n\n\t' + titoli.get_text().upper() + '\n'
        #str_group =
        print(colored('\n\n\t' + stdfunc.removeSpace(list(titoli.get_text())).upper(),'magenta', attrs=['bold','underline']))
        for count in range(1, 5):
            ...
            print(colored(lst_teamTag[i],'blue'))
            print(colored(str_fileOut,'grey'))
            str_fileOut = ''
            i += 1
...
def statistiche(link):
    ...
    print(colored('Rosa: ', 'magenta', attrs=['bold']))
    for index in range(0, len(lst_rolePlayer)):
        ...
        print(colored(str_number + '.' + str_name + str_role + str_match,'grey'))
    print('\n* Giocatore lista B')
    print('\n')
    print(colored('Partite giocate e da giocare', 'blue'))
    index = 0
    for i in range(0, len(lst_homeScore)-4):
        ...
        index += 2
    print('\n')
    print(colored('Statistiche dettagliate','blue'))
...
def stampaSquadre():
    ...
    for i in range(0,8):
        for j in range(0,4):
            if (index + 1) < 10:
                aus = ' ' + colored(str(index + 1),'yellow') + ': ' + colored(stdfunc.removeSpace(list(lst_teamTag[index])),'grey')
            else:
                aus = colored(str(index + 1),'yellow') + ': ' + colored(stdfunc.removeSpace(list(lst_teamTag[index])),'grey')

            for j in range(len(aus), 50):
                aus = aus + ' '
            index += 1
            str_out = str_out + aus
        print(str_out)
    ...
    try:
        ...
        if int(scelta) > 0 & int(scelta) < 32:
            print(colored('\t\t' + stdfunc.removeSpace(list(lst_team[int(scelta) - 1].get_text())) + '\n', 'blue'))
...
def stampaMenu():
    ...
    print(colored('1 - Stampa classifica dettagliata','green'))
    print(colored('2 - Stampa squadre','green'))
    scelta = input()
    try:
        if scelta == '1':
            stampaClassifica()
